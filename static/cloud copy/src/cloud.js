...
var tabs = d3.select("#presets").selectAll("a")
    .data([
      {fetcher: "http://en.wikipedia.org/wiki/{word}", name: "Twitter"},
...
function getURL(url, word, callback) {
  ...
  if (matchTwitter.test(url)) {
    ...
    iframe.attr("src", "https://arpecop.net/static/cloud/jsonp.html?" + encodeURIComponent(url));
...
function proxy(url, callback) {
  d3.text("https://arpecop.net/static/cloud/jsonp.html?url=" + encodeURIComponent(url), callback);
}
...
function downloadPNG() {
  ...
  $.post("http://aws.arpecop.net/cloud/", { str: canvas.toDataURL("image/png"), token: $.gvar.token},
   function(data) {
      $('#loading,#lislider').hide();
      $('#shareme').html('<a href="#"><i class="icon-github-alt"></i> Thank You ;)!</a>')
   });
...
function load(d, f) {
 parseText(' is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry  standard dummy text');
}
