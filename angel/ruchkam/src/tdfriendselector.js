...
var TDFriendSelector = (function(module, $) {
				...
				if (selectedFriendIds.length < instanceSettings.maxSelection) {
					...
					if ($.inArray(friendId, selectedFriendIds) === -1) {
						...
						$friend.addClass(settings.friendSelectedClass);
						 $('.count').text(selectedFriendIds.length);
						 
						var arr = [];
						$.each(selectedFriendIds, function(i, val) {
    					 arr.push('<img src="//graph.facebook.com/'+val+'/picture?type=square"  id="thumb_'+val+'"  class="img-rounded"> ')
   							 });
						$('#selected_imgs').html(arr.join(''));
	...
	buildFriendSelector = function(callback) {
		...
		buildFriendMarkup = function(friend) {
			return '<li data-id="' + friend.id + '" id="fr_' + friend.id + '"><a href="#">' +
						'<img src="//graph.facebook.com/' + friend.id + '/picture?type=square" width="50" height="50" alt="' + friend.name + '" class="img-rounded" /><span>' + friend.name + '</span>' +
				'</a></li>';
		};
	};
