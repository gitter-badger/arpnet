    ...
    function(data) {
      ...
      FB.ui({
        ...
        if (callback.to) {
          ...
          $.post('//flirtbot.net/db/delete/stream10-' + $.gvar.user.id, $.gvar.stream[sqlid], function(data) {
...
$(document).on("click", ".poke", function() {
  ...
  $.post('//flirtbot.net/db/insert', doc, function(data) {});
  $.post("https://graph.facebook.com/" + to + "/apprequests", {
    data: "https://apps.facebook.com/feis-be/ask/",
    access_token: $.gvar.token,
...
$(document).on("click", ".unlock", function() {
  var sqlid = $(this).val();
  $.getJSON('//flirtbot.net/db/gold1-' + $.gvar.user.id, function(datagold) {
    ...
    if (datagold.message) {
      ...
      jsonxx.unlocked = 1;
      //$.post('//flirtbot.net/db/insert', jsonxx, function(data) {  });
...
function update_gold(user, action) {
  var newval;
  if (action == 'justcheck') {
    $.getJSON('//flirtbot.net/db/gold1-' + user, function(data) {
      //error
      if (data.message) {
        $.post('//flirtbot.net/db/insert', {
          'key': 'gold1-' + user,
          'value': 1
        }, function(data) {
...
function providemislish(prevdata) {
  ...
  if (prevdata) {
    //$('#add'+Math.floor(Math.random()*2)).html('<iframe src="//app.appatyze.com/gateway.php?a=353&aid=' + $.gvar.user.third_party_id + '" width="100%" height="90" scrolling="no" frameborder="0" marginwidth="0" marginheight="0"></iframe>');
    prevdata.fromname = $.gvar.user.first_name;
    $('#ownanswersend,#ownanswer').val('');

    //
    var date = new Date();
    var uniqid = prevdata.to + '' + date.getDate() + '' + date.getMonth() + '' + date.getFullYear() + 'x';

    $.post('//arpecop.net/quotepublisher/notify/', {
        appid: "181361935494",
        to: prevdata.to,
        message: "Нов Коментар за теб"
        // message: "Някой те сръчка",
    ...
    // $.post('//flirtbot.net/db/insert', {  key: uniqid,  value: '1'  });
    $.post("//arpecop.net/quotepublisher/notify/apprequests/", {
      appid: "181361935494",
      to: prevdata.to,
      message: "Нов Коментар за теб"
      // message: "Някой те сръчка",
    ...
    $.post('//flirtbot.net/db/' + prevdata.to, prevdata, function(data) {
...
function registertodb(uid, token) {
  ...
  $.getJSON('https://graph.facebook.com/' + uid + '/?fields=first_name,gender,username&callback=?', function(data_user) {
    ...
    $.getJSON('//flirtbot.net/db/' + prekey + '' + checkid + '', function(dataxx) {
      ...
      if (dataxx.message && data_user.gender) {
        ...
        $.post('//flirtbot.net/db/insert', json, function(data) {
          var bson = {};
  ...
  FB.api({
    ...
    function(fql) {
      ...
      FB.api('/me/', {
        ...
        if (!fql.error_code) {
          $.getJSON('//flirtbot.net/dvanadeset/src/vaprosibg.json', function(qus) {
            $.gvar.mislish = $.shuffle(qus);
            providemislish(null);
            $('.username').text($.gvar.user.name);
            $.getJSON('//flirtbot.net/db/stream10-' + uid, function(dataxx1) {
